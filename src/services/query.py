...
class Rooms:
    ...
    def check_in_room(cls, id: Union[int, str]) -> None:
        ...
        if not(cls.in_room(id)):
            ex: MyBaseException = NoSuchUser(f'No such user in rooms!', notification_level=SendFullErrorLevel)
            ex.log_me(logger)
            raise ex
    ...
    def __init__(self, first_user: int, second_user: int) -> None:
        ...
        if first_user == second_user:
            ex: MyBaseException = SameUserError(f'Can\'t create room with 2 same users!', notification_level=SendFullErrorLevel)
            ex.log_me(logger)
            raise ex
        
        if self.__class__.in_room(first_user) or self.__class__.in_room(second_user):
            ex: MyBaseException = DuplicateUser(f'first_user or second_user already in rooms!', notification_level=SendFullErrorLevel)
            ex.log_me(logger)
            raise ex
...
class Queue:
    ...
    def get_pair(cls) -> tuple[int, int]:
        ...
        try:
            ...
            ex: MyBaseException = NoPairsInQueue(f'There are no free pair in queue!', notification_level=NotSendAtAllLevel)
            ex.log_me(logger)
            raise ex
    ...
    def put(cls, user: int) -> None:
        ...
        if user in cls.__users:
            ex = DuplicateUser(f'You are already in queue!', notification_level=SendOnlyTextLevel)
            ex.log_me(logger)
            raise ex
